...
class Speech(TFBase):
  ...
  def standardL2Model(self, args, infer=False):
    ...
    inputs_2d = tf.split(self.input_data, args.seq_length, axis=1)
    inputs_2d = [tf.squeeze(input_, [1]) for input_ in inputs_2d]

    # outputs, states = tf.contrib.legacy_seq2seq.rnn_decoder(inputs_2d, self.initial_state, self.network, loop_function=None, scope='rnnlm')
    outputs, states = tf.contrib.legacy_seq2seq.attention_decoder(inputs_2d, self.initial_state, self.input_data, self.network, loop_function=None, scope='rnnlm')
